...
async function existingCategories() {
    ...
    for (const locale of Object.keys(LOCALE_STRINGS)) {
        ...
        try {
            ...
            entries.filter((entry) => entry.isDirectory()).forEach((entry) => categories.add(entry.name));
        } catch {
...
${
    docBlock
        ? `<h3 class="mb-1 mt-3">${strings.documentationHeading(data.title)}</h3>
...
async function askCategory() {
    while (true) {
        const answer = (await rl.question('Category [s = scientific-computing, v = visualisation]: '))
            .trim()
            .toLowerCase();
...
function buildTableRow(locale, category, anchor) {
    ...
    if (category === 'scientific-computing') {
        ...
        return ['        <tr>', `            ${firstCell}`, '            <td></td>', '        </tr>'].join('\n');
    }
    const firstCell = `<td${firstColumnClass}></td>`;
    const secondCell = `<td>${anchor}</td>`;
    return ['        <tr>', `            ${firstCell}`, `            ${secondCell}`, '        </tr>'].join('\n');
}
...
function insertBeforeSmallVisualSelect(content, block) {
    ...
    const marker = '<div class="col-6 col-diamond container-select container-select-2 container-rtl" align="right">';
...
function updateScientificPart3(content, locale, data) {
    ...
    const highestIndex = Math.max(getHighestNumericOption(content, 'A'), getHighestNumericOption(content, 'B'));
...
function updateVisualPart3(content, locale, data) {
    ...
    const highestLetter = Math.max(getHighestLetterOption(content, 'A'), getHighestLetterOption(content, 'B'));
